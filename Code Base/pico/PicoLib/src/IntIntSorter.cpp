...
namespace pico
{
  ...
  void IntIntSorter::executeOnGPUAsync(void * const keys, void * const vals, const int numKeys, int & numUniqueKeys, int ** keyOffsets, int ** valOffsets, int ** numVals)
  {
    ...
    */
    ...
    // 2 - A = gpuUniqueFlags
    picoIntIntSorterMarkUnique(gpuInputKeys, gpuUniqueFlags, numKeys);
    ...
    // 3 - D = gpuValOffsets
    cudacpp::Runtime::sync();
    picoIntIntSorterFindOffsets(gpuInputKeys, gpuUniqueFlags, gpuValOffsets, gpuInputVals, gpuValOffsets, numKeys, numUniqueKeys);
    ...
    *valOffsets = reinterpret_cast<int * >(cudacpp::Runtime::mallocHost(numUniqueKeys * sizeof(int)));
    ...
    // 4 - F = gpuInputVals
    picoIntIntSorterSetCompactedKeys(gpuInputKeys, gpuUniqueFlags, gpuInputVals, numUniqueKeys);
