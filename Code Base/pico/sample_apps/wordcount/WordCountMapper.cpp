executeOnGPUAsync#include <WordCountMapper.h>
...
pico::EmitConfiguration WordCountMapper::getEmitConfiguration(pico::Chunk * const chunk) const
{
  const int keySize = sizeof(int);
  const int valSize = sizeof(int);
  dim3 blockSize(256, 1, 1);
  dim3 gridSize((numUniqueWords + blockSize.x - 1) / blockSize.x, 1, 1);
  return pico::EmitConfiguration::createGridConfiguration(gridSize.x * blockSize.x * keySize,
...
void WordCountMapper::executeOnGPUAsync(pico::Chunk * const chunk, picoGPUConfig & picoGPUConfig, void * const gpuMemoryForChunk,
                                        cudacpp::Stream * kernelStream, cudacpp::Stream * memcpyStream)
{
  int commRank, commSize;
  MPI_Comm_rank(MPI_COMM_WORLD, &commRank);
  MPI_Comm_size(MPI_COMM_WORLD, &commSize);
  pico::PreLoadedFixedSizeChunk * fsChunk = dynamic_cast<pico::PreLoadedFixedSizeChunk * >(chunk);
...
void WordCountMapper::executeOnCPUAsync(pico::Chunk * const chunk, picoCPUConfig & picoCPUConfig)
{
}
